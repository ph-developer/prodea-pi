...
void main() {
  ...
  group('init', () {
    test(
      ...
      () async {
        ...
        whenReaction(
            (_) => store.requestedDonations, requestedDonationsChanged);
        whenReaction(
            (_) => store.availableDonations, availableDonationsChanged);
  ...
  group('setDonationAsUnrequested', () {
    test(
      ...
      () async {
        ...
        // assert
        verify(() => setDonationAsUnrequestedMock(tDonationWithPhoto))
            .called(1);
      },
    );
  });

  group('setDonationAsCanceled', () {
    test(
