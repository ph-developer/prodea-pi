...
void main() {
  ...
  test('deve emitir true quando tiver conexão com a internet.', () {
    // arrange
  ...
  test('deve emitir false quando não tiver conexão com a internet.', () {
    // arrange
  ...
  test('deve emitir false e notificar quando algum erro ocorrer.', () async {
    // arrange
    when(networkServiceMock.isConnected).thenThrow(InternetConnectionFailure());
    // act
    final stream = usecase(1);
    // assert
    expect(stream, emitsInOrder([false, false, false]));
